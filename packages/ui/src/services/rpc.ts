...
export async function sendTransaction(theSignedTx: any) {
...
export async function getLiveCell(out_point: any, with_data: any) {
  const params = [out_point, with_data]
  const json = await callRpc("get_live_cell", params)
  return json
}

export async function indexLockHash(lockHash: any, indexFrom: any) {
...
export async function getLiveCellsByLockHash(lockHash: string, page: any, per: any) {
  const params = [lockHash, page, per]
  const json = await callRpc("get_live_cells_by_lock_hash", params)
  return json
}

export async function getTransactionsByLockHash(lockHash: any, page: any, per: any) {
  const params = [lockHash, page, per]
  const json = await callRpc("get_transactions_by_lock_hash", params)
  return json
}

export async function getCapacityByLockHash(lockHash: string, page: any, per: any) {
  const params = [lockHash, page, per]
  const json = await callRpc("get_capacity_by_lock_hash", params)
  return json
}

export function getCellsSummary(cells: any) {
  const result = {
    capacity: new BN(0),
    inuse: new BN(0),
    free: new BN(0),
