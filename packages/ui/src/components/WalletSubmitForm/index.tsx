...
const WalletSubmitForm = ({ mnemonic, onBack }: WalletSubmitFormProps) => {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value, dataset: { fieldName } } = e.target
    dispatch({ fieldName: fieldName as keyof FormState, value })
  }

  const rePasswrodError = form.password && form.rePassword && form.password !== form.rePassword
  ...
  return (
    <form className={styles.container} onSubmit={handleConfirm}>
      ...
      <div className={styles.fields}>
        <TextField label="Wallet Name" fieldName="name" value={form.name} onChange={handleInput} disabled={submitting} />
        <TextField label="Password" fieldName="password" value={form.password} onChange={handleInput} type="password" disabled={submitting} />
        <TextField
