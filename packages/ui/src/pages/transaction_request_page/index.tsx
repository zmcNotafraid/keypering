...
import { Button, Flex, Icon, Modal, NavBar, Toast } from 'antd-mobile'
import { Bytes, RawTransaction, Script, scriptToAddress } from '@keyper/specs'
...
const InputOutput = ({ capacity, lock, type, data }: Cell) => {
  ...
  return (
    <div className={styles.inputOutput}>
      ...
      <Tag type={(type && 'primary') || 'blank'} className={styles.tag}>
        type
      </Tag>
      <Tag type={(data !== '0x' && 'primary') || 'blank'} className={styles.tag}>
        data
      </Tag>
...
class TransactionRequestPage extends React.Component<any, any> {
  ...
  handleApprove = async () => {
    Modal.prompt(
      'Password',
      'Sign to broadcast the transaction',
      [
        {
          text: 'Cancel',
        },
  ...
  handleConfirm = async (password: string) => {
    ...
    const { lockHash, tx, config, token } = params
  ...
  render() {
    ...
    const { tx, description } = params as TransactionRequestParams
